...
class canvas(widget):
    """
    Canvas class which allows a group of widgets to be displayed together.

    :param name: The name of the canvas (optional)
    :type name: str
    :param size: The size to make the canvas in pixels (x, y)
    :type size: (int, int)
    :param dataset: The dataset to use for this canvas
    ...
    def __init__(self, name=None, size=None, dataset=None):
        super().__init__(name=name, size=size, dataset=dataset)
    ...
    def append(self, item=None, offset=None, just="lt", z=ZSTD):
        ...
        :param offset:  The distance from the placement point to offset the
            offset the object when placing it (x direction, y direction)
        :type offset: (int, int)
        :param just: Set the justification for the placement point
        :type just: str
        :param z: The z order for the item.  Higher z order values get placed
            above lower ones.
        :type z: int
        """
        assert (
    ...
    def _render(self, force=False, *args, **kwargs):
        ...
        if changed:
            ...
            for img, off, just in results:
                self._place(
                    retainImage=True, wImage=img, offset=off, just=just
                )
...
class windows(canvas):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            name=name,
            size=(
                max(size[0], self._defaultCanvas.size[0]),
                max(size[1], self._defaultCanvas.size[1]),
            ),
            dataset=dataset,
            *args,
            **kwargs,
    ...
    def _render(self, force=False, *args, **kwargs):
        ...
        self._cooling = {
            k: v for k, v in self._cooling.items() if v[0] + v[1] >= ct
        }

        # Remove any window that's minDuration period has ended
        self._minTimer = {
            k: v for k, v in self._minTimer.items() if v[0] + v[1] >= ct
        }

        # Remove any window that's Duration period has ended
        self._duration = {
            k: v for k, v in self._duration.items() if v[0] + v[1] >= ct
        }
        ...
        for w, o, j, z, dur, min, cool, cond in self._windows:
            ...
            if (
                (winActive and w not in self._cooling and w not in self._inUse)
                or (
                    winActive
                    and w in self._inUse
                    and (dur == 0 or w in self._duration)
                )
                ...
                if w not in self._inUse:
                    ...
                    inUse = True
                renderList.append(
                    (w, w.render(force=inUse or force)[0], o, j, z)
                )
            else:
                if w in self._inUse:
        ...
        if not self._inUse:
            renderList.append(
                (
                    self._defaultCanvas,
                    self._defaultCanvas.render(force=True)[0],
                    (0, 0),
                    "lt",
                    canvas.ZSTD,
                )
...
class sequence(canvas):  # noqa: D101
    ...
    def activeCanvas(self):
        ...
        :returns: the currently active Canvas or None if no canvas is active and whether the activeCanvas is newly activated
    ...
    def _activeCanvas(self):
        return (
            self._dataset.eval(self._canvases[self._currentCanvas][3])
            or self._ticksRemainingCurrentCanvas()
        )
    ...
    def _ticksRemainingCurrentCanvas(self):
        """
        Return ticks remaining for current canvas.

        Determine whether the current Canvas has ticks remaining (minimum time has not expired)

        :returns: True if minimum time has not expired else False
        :rtype: bool
        """
